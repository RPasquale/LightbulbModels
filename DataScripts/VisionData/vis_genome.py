...
# Custom Dataset Class for Visual Genome Data
...
def shard_data(features, bboxes, labels, shard_size, local_dir):
    ...
    for i in range(num_shards):
        ...
        filename = os.path.join(local_dir, f"visgenome_shard_{i:06d}.npz")
        write_datafile(filename, shard_features, shard_bboxes, shard_labels)

def main():
    # Parameters
    local_dir = "C:/Users/Admin/DATAS/visgenome_features"
    shard_size = 10000  # Adjust this value based on your requirement
    os.makedirs(local_dir, exist_ok=True)

    # Load the dataset
    dataset = load_dataset("ranjaykrishna/visual_genome", "objects_v1.2.0", split='train', trust_remote_code=True)

    # Create the dataset and dataloader
    vg_dataset = VisualGenomeDataset(dataset, transform=transform)
    vg_dataloader = DataLoader(vg_dataset, batch_size=2, shuffle=True, collate_fn=custom_collate_fn)
    ...
    # Extract features from Visual Genome data
    features, bboxes, labels = extract_features(vg_dataloader, resnet_model, max_objects=10)
